            ...
            if any((key in s for key in filter_keys)) or progress_pct is not None:
                if s != last_shown_msg:
                    if progress_pct is not None:
                        sys.stdout.write(
                            f"\r{CYAN}{progress_bar(progress_pct, width=28)} {progress_pct:3d}%{RESET}"
                        )
                        sys.stdout.flush()
                    else:
                        sys.stdout.write("\r" + " " * 120 + "\r")
    ...
    try:
        while True:
            ...
            if "Setting up" in s or "Unpacking" in s:
                installed_count += 1
                current_pct = (
                    min(100, int(installed_count / total * 100)) if total > 0 else 100
                )
            ...
            if current_pct > 0:
                sys.stdout.write(
                    f"\r{CYAN}{progress_bar(current_pct, width=28)} {label} ({installed_count}/{total}) {_SPINNER_FRAMES[spinner_idx]}{RESET}"
                )
            else:
                sys.stdout.write(
                    f"\r{CYAN}{label}... {_SPINNER_FRAMES[spinner_idx]}{RESET}"
                )
            sys.stdout.flush()
    finally:
        sys.stdout.write("\r" + " " * 120 + "\r")
...
def ensure_python_venv_installed():
    ...
    try:
        ...
        if result == 0:
            ...
            print(
                f"{RED}Please run manually: sudo apt install -y {' '.join(packages_to_install)}{RESET}"
            )
            return False
...
def apply_update_and_restart(extract_dir: Path):
    install_dir = (
        Path(sys.executable).parent if getattr(sys, "frozen", False) else PROJECT_DIR
    )
...
def standalone_update_check():
    ...
    def progress_cb(downloaded, total, pct):
        sys.stdout.write(
            f"\r{CYAN}{progress_bar(pct, width=28)} {downloaded // (1024 * 1024)}MB/{total // (1024 * 1024)}MB{RESET}"
        )
        sys.stdout.flush()
...
def main():
    ...
    if start_py.exists():
        if SENTINEL.exists():
            print(
                f"{DIM}{step_label(3, 3, 'Environment ready,bypassing checks')}{RESET}"
            )

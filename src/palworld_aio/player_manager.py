        ...
        for p in g["value"]["RawData"]["value"].get("players", []):
            ...
            if uid == uid_clean:
                ...
                lastseen = (
                    "Unknown"
                    if last is None
                    else format_duration_short((tick - last) / 10000000.0)
                )
    ...
    for entry in char_map:
        ...
        if uid == uid_clean:
                    ...
                    if "StatusName" in status_item and "StatusPoint" in status_item:
                        if isinstance(status_item["StatusPoint"], dict):
                            if "value" in status_item["StatusPoint"]:
                                if (
                                    isinstance(status_item["StatusName"], dict)
                                    and "value" in status_item["StatusName"]
                                ):
                                    stat_name = status_item["StatusName"]["value"]
                                    if stat_name in stat_changes:
                                        status_item["StatusPoint"]["value"] = (
            ...
            if "UnusedStatusPoint" in sp_val:
                if (
                    isinstance(sp_val["UnusedStatusPoint"], dict)
                    and "value" in sp_val["UnusedStatusPoint"]
                ):

...
<<<<<<< Updated upstream
    ...
    from palworld_aio.func_manager import (
        ...
        reset_selected_player_timestamp,
        detect_and_trim_overfilled_inventories,
        unlock_all_technologies_for_player,
        unlock_all_lab_research_for_guild,
        modify_container_slots,
        fix_illegal_pals_in_save,
        repair_structures,
    )
    from palworld_aio.guild_manager import (
        move_player_to_guild,
        rebuild_all_guilds,
        make_member_leader,
        rename_guild,
        max_guild_level,
...
class DetachedStatusWindow(QWidget):
    ...
    def _apply_fallback_styles(self):
        if self.is_dark:
            ...
            accent_color = "#1e3a8a"
        self.setStyleSheet(
            f"QWidget {{ background: {bg_gradient}; color: {txt_color}; font-family: 'Segoe UI',Roboto,Arial; }}"
        )
        self.container.setStyleSheet(
            f"#mainContainer {{ background: {glass_bg}; border-radius: 10px; border: 1px solid {glass_border}; }}"
        )
    ...
    def setup_status_ui(self):
        ...
        self.title_label.setStyleSheet(
            f"font-weight: bold; font-size: 14px; color: {txt_color};"
        )
    ...
    def append_message(self, text):
        ...
        if document.blockCount() > 500:
            ...
            cursor.movePosition(
                QTextCursor.Down, QTextCursor.KeepAnchor, document.blockCount() - 500
            )
    ...
    def closeEvent(self, event):
        if self.parent and hasattr(self.parent, "user_settings"):
            try:
                self.parent.user_settings["console_window_geometry"] = (
                    self.save_geometry()
                )
...
class StatusBarStream(QObject):
    ...
    def detach(self):
        if not self.detached:
            ...
            saved_geo = (
                self.parent.user_settings.get("console_window_geometry")
                if self.parent and hasattr(self.parent, "user_settings")
                else None
            )
            if saved_geo:
            ...
            self.detach_window.fade_animation = QPropertyAnimation(
                self.detach_window, b"windowOpacity"
            )
...
class UpdateChecker(QThread):
    ...
    def run(self):
        try:
            from palworld_aio.updater import (
                check_for_updates,
                SourceUpdater,
                StandaloneUpdater,
            ...
            self.update_checked.emit(
                not available, latest, branch if not is_standalone() else "stable"
            )
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def _setup_players_tab(self):
        ...
        self.players_panel.tree.customContextMenuRequested.connect(
    ...
    def _setup_guilds_tab(self):
        ...
        self.guilds_panel.tree.customContextMenuRequested.connect(
            self._show_guild_context_menu
        )
        ...
        self.guild_members_panel.tree.customContextMenuRequested.connect(
            self._show_guild_member_context_menu
        )
    ...
    def _setup_bases_tab(self):
        ...
        self.bases_panel.tree.customContextMenuRequested.connect(
            self._show_base_context_menu
        )
    ...
    def _setup_exclusions_tab(self):
        ...
        self.excl_players_panel = SearchPanel(
            "deletion.exclusions.player_label", ["deletion.excluded_player_uid"], [300]
        )
        ...
        self.excl_guilds_panel = SearchPanel(
            "deletion.exclusions.guild_label", ["deletion.excluded_guild_id"], [300]
        )
        ...
        self.excl_bases_panel = SearchPanel(
            "deletion.exclusions.base_label", ["deletion.excluded_bases"], [300]
        )
...
<<<<<<< Updated upstream
        menu_actions = {
            "file": [
                ...
                (
                    t("menu.file.load_worldoption") if t else "Load WorldOption",
                    self._load_worldoption,
                ),
                (
                    t("menu.file.load_levelmeta") if t else "Load LevelMeta",
                    self._load_levelmeta,
                ),
                (
                    t("menu.file.save_changes") if t else "Save Changes",
                    self._save_changes,
                ),
                (
                    t("menu.file.rename_world") if t else "Rename World",
                    self._rename_world,
                ),
            ],
            "functions": [
                (
                    t("deletion.menu.delete_empty_guilds")
                    if t
                    else "Delete Empty Guilds",
                    self._delete_empty_guilds,
                ),
                (
                    t("deletion.menu.delete_inactive_bases")
                    if t
                    else "Delete Inactive Bases",
                    self._delete_inactive_bases,
                ),
                (
                    t("deletion.menu.delete_duplicate_players")
                    if t
                    else "Delete Duplicate Players",
                    self._delete_duplicate_players,
                ),
                (
                    t("deletion.menu.delete_inactive_players")
                    if t
                    else "Delete Inactive Players",
                    self._delete_inactive_players,
                ),
                (
                    t("deletion.menu.delete_unreferenced")
                    if t
                    else "Delete Unreferenced Data",
                    self._delete_unreferenced,
                ),
                (
                    t("deletion.menu.delete_non_base_map_objs")
                    if t
                    else "Delete Non-Base Map Objects",
                    self._delete_non_base_map_objs,
                ),
                (
                    t("deletion.menu.delete_all_skins") if t else "Delete All Skins",
                    self._delete_all_skins,
                ),
                (
                    t("deletion.menu.unlock_private_chests")
                    if t
                    else "Unlock Private Chests",
                    self._unlock_private_chests,
                ),
                (
                    t("deletion.menu.remove_invalid_items")
                    if t
                    else "Remove Invalid Items",
                    self._remove_invalid_items,
                ),
                (
                    t("deletion.menu.remove_invalid_structures")
                    if t
                    else "Remove Invalid Structures",
                    self._remove_invalid_structures,
                ),
                (
                    t("deletion.menu.repair_structures")
                    if t
                    else "Repair All Structures",
                    self._repair_structures,
                ),
                (
                    t("deletion.menu.remove_invalid_pals")
                    if t
                    else "Remove Invalid Pals",
                    self._remove_invalid_pals,
                ),
                (
                    t("deletion.menu.remove_invalid_passives")
                    if t
                    else "Remove Invalid Passives",
                    self._remove_invalid_passives,
                ),
                (
                    t("deletion.menu.fix_illegal_pals") if t else "Fix Illegal Pals",
                    self._fix_illegal_pals,
                ),
                (
                    t("deletion.menu.reset_missions") if t else "Reset Missions",
                    self._reset_missions,
                ),
                (
                    t("deletion.menu.reset_anti_air")
                    if t
                    else "Reset Anti-Air Turrets",
                    self._reset_anti_air,
                ),
                (
                    t("deletion.menu.reset_dungeons") if t else "Reset Dungeons",
                    self._reset_dungeons,
                ),
                (
                    t("deletion.menu.paldefender") if t else "PalDefender Commands",
                    self._open_paldefender,
                    "separator_after",
                ),
                (
                    t("deletion.menu.fix_timestamps")
                    if t
                    else "Fix All Negative Timestamps",
                    self._fix_all_timestamps,
                    "separator_after",
                ),
                (
                    t("base.export_all") if t else "Export All Bases",
                    self._export_all_bases,
                ),
                (
                    t("guild.menu.rebuild_all_guilds") if t else "Rebuild All Guilds",
                    self._rebuild_all_guilds,
                ),
                (
                    t("guild.menu.move_selected_player_to_selected_guild")
                    if t
                    else "Move Player to Guild",
                    self._move_player_to_guild,
                ),
                (
                    t("deletion.menu.trim_overfilled_inventories")
                    if t
                    else "Trim Overfilled Inventories",
                    self._trim_overfilled_inventories,
                ),
                (
                    t("modify_container_slots") if t else "Modify Container Slots",
                    self._modify_container_slots,
                ),
                "separator_after",
            ],
            "player_editing": [
                (
                    t("player.edit_tech_points") if t else "Edit Tech Points",
                    self._edit_player_tech_points,
                ),
                (
                    t("player.edit_stats") if t else "Edit Player Stats",
                    self._edit_player_stats,
                ),
                "separator_after",
            ],
            "maps": [
                ...
                (
                    t("deletion.menu.generate_map") if t else "Generate Map",
                    self._generate_map,
                ),
            ],
            "exclusions": [
                (
                    t("deletion.menu.save_exclusions") if t else "Save Exclusions",
                    self._save_exclusions,
                )
            ],
            "languages": [
                (
                    t(f"lang.{code}") if t else code,
                    partial(self._change_language, code),
                    {
                        "en_US": "🇺🇸",
                        "zh_CN": "🇨🇳",
                        "ru_RU": "🇷",
...
>>>>>>> Stashed changes
    ...
    def _build_aio_menu(self):
        """Build the AIO menu with update options"""
        menu_items = [
            (
                t("aio.menu.check_updates") if t else "Check for Updates...",
                self._check_for_updates,
            ),
            (
                t("aio.menu.update_settings") if t else "Update Settings...",
                self._show_update_settings,
            ),
        ]
    ...
    def _on_manual_update_check(self, ok, latest, branch):
        if ok and latest is None:
            self._show_info(
                t("update.error.title") if t else "Update Check Failed",
                t("update.error.network")
                if t
                else "Could not check for updates. Please try again later.",
            )
        elif ok:
            self._show_info(
                t("update.up_to_date.title") if t else "Up to Date",
                t("update.up_to_date.message")
                if t
    ...
    def _show_update_dialog(self, latest, branch):
        ...
        info_label = QLabel(
            f"{t('update.current') if t else 'Current'}: {current_version}\n{t('update.latest') if t else 'Latest'}: {latest}"
        )
    ...
    def _perform_source_update(self, dialog):
        """Perform git-based update for source mode"""
        ...
        if success:
            ...
            msg_box.setWindowTitle(
                t("update.complete.title") if t else "Update Complete"
            )
            msg_box.setText(
                t("update.restart_prompt")
                if t
                else "The application needs to restart to apply the update. Restart now?"
            )
            msg_box.addButton(
                t("button.restart") if t else "Restart Now", QMessageBox.AcceptRole
            )
            msg_box.addButton(
                t("button.later") if t else "Later", QMessageBox.RejectRole
            )
            ...
            self._show_error(
                t("update.failed.title") if t else "Update Failed", message
            )
            self.progress_bar.setVisible(False)
            self.status_label.setText("")

    def _perform_standalone_update(self, dialog, version):
        """Perform download-based update for standalone mode"""
        ...
        if not archive_path:
            self._show_error(
                t("update.failed.title") if t else "Update Failed",
                t("update.download_failed") if t else "Failed to download update.",
            )
            self.progress_bar.setVisible(False)
            self.status_label.setText("")
            return
        ...
        if not extracted:
            self._show_error(
                t("update.failed.title") if t else "Update Failed",
                t("update.extract_failed") if t else "Failed to extract update.",
            )
            self.progress_bar.setVisible(False)
            self.status_label.setText("")
            return
        ...
        if msg_box.exec() == QMessageBox.AcceptRole:
            if self.updater.apply_and_restart():
                ...
                self._show_error(
                    t("update.failed.title") if t else "Update Failed",
                    t("update.apply_failed") if t else "Failed to apply update.",
                )
        else:
    ...
    def _show_update_settings(self):
        ...
        self.check_updates_cb = QCheckBox(
            t("update.check_auto") if t else "Automatically check for updates"
        )
        ...
        if is_standalone():
            ...
            self.auto_update_cb = QCheckBox(
                t("update.auto_update") if t else "Auto-update when available"
            )
            self.auto_update_cb.setChecked(settings.get("auto_update", True))
            layout.addWidget(self.auto_update_cb)
        else:
            dialog.setFixedSize(400, 200)
            self.git_pull_cb = QCheckBox(
                t("update.git_pull") if t else "Allow git pull updates"
            )
    ...
    def _save_update_settings_dialog(self, dialog):
        """Save update settings from dialog"""
        if is_standalone():
            settings = {
            ...
            settings = {
                "check_updates": self.check_updates_cb.isChecked(),
                "git_pull": self.git_pull_cb.isChecked(),
            }
        save_update_settings(settings)
        dialog.close()
        self._show_info(
            t("settings.saved.title") if t else "Settings Saved",
            t("settings.saved.message") if t else "Update settings saved.",
        )
    ...
    def _create_message_box(self, icon=QMessageBox.Information):
        ...
        msg_box.setWindowFlags(
            Qt.Dialog | Qt.WindowType.Window | Qt.WindowStaysOnTopHint
        )
        ...
        if os.path.exists(user_cfg_path):
            try:
                with open(user_cfg_path, "r") as f:
                    ...
                    self.is_dark_mode = (
                        self.user_settings.get("theme", "dark") == "dark"
                    )
            except Exception as e:
    ...
    def _apply_fallback_styles(self):
        border_color = (
            "rgba(70,70,70,1.0)" if self.is_dark_mode else "rgba(70,70,70,1.0)"
        )
        from PySide6.QtWidgets import QApplication
...
>>>>>>> Stashed changes
    ...
    def _detach_status(self):
        if self.status_stream:
            if self.status_stream.detached:
                ...
                self.status_stream.detach()
        self.user_settings["console_detached"] = (
            self.status_stream.detached if self.status_stream else False
        )
        self._save_user_settings()
    ...
    def _on_detach_state_changed(self, detached):
        ...
        if detach_btn:
            detach_btn.setText(
                t("console.reattach") if detached else t("console.detach")
            )
        ...
        try:
            if not ok and latest:
                ...
                self.status_bar.showMessage(
                    f"{(t('update.current') if t else 'Current')}: {tools_version}{branch_text} | {(t('update.latest') if t else 'Latest')}: {latest} - Click version chip to update",
                    0,
                )
            else:
    ...
    def _on_load_finished(self, success):
        if success:
            ...
            self.status_bar.showMessage(
                t("status.loaded") if t else "Save loaded successfully", 5000
            )
            msg_box = self._create_message_box(QMessageBox.Information)
            ...
            self.status_bar.showMessage(
                t("status.load_failed") if t else "Failed to load save", 5000
            )
            msg_box = self._create_message_box(QMessageBox.Critical)
    ...
    def _on_save_finished(self, duration):
        self.status_bar.showMessage(
            f"{(t('status.saved') if t else 'Save completed')}({duration:.2f}s)", 5000
        )
        msg_box = self._create_message_box(QMessageBox.Information)
        msg_box.setWindowTitle(t("success.title"))
    ...
    def _refresh_players(self):
        ...
        for uid, name, gid, lastseen, level in players:
            ...
            self.players_panel.add_item(
                [display_name, lastseen, level, pals, uid, gname, gid, glevel]
            )
    ...
    def _show_warnings(self):
        warnings = [
            (
                t("notice.backup")
                if t
                else "WARNING: ALWAYS BACKUP YOUR SAVES BEFORE USING THESE TOOLS!",
                {},
            ),
            (
                t("notice.patch", game_version=get_versions()[1])
                if t
                else "MAKE SURE TO UPDATE YOUR SAVES AFTER EVERY GAME PATCH!",
                {},
            ),
            (
                t("notice.errors")
                if t
                else "IF YOU DO NOT UPDATE YOUR SAVES AFTER A PATCH,YOU MAY ENCOUNTER ERRORS!",
                {},
            ),
        ]
    ...
    def closeEvent(self, event: QCloseEvent):
        if self.status_stream and self.status_stream.detach_window:
            try:
                self.user_settings["console_window_geometry"] = (
                    self.status_stream.detach_window.save_geometry()
                )
                self._save_user_settings()
            except (RuntimeError, AttributeError):
                pass
    ...
    def _show_player_context_menu(self, pos):
        ...
        menu.add_action(
            self._create_action(
                t("deletion.ctx.add_exclusion"),
                lambda: self._add_exclusion("players", item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("deletion.ctx.remove_exclusion"),
                lambda: self._remove_exclusion("players", item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("deletion.ctx.delete_player"),
                lambda: self._delete_player(item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.rename.menu"),
                lambda: self._rename_player(item.text(4), item.text(0)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.viewing_cage.menu"),
                lambda: self._unlock_viewing_cage(item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.reset_timestamp.menu") if t else "Reset Timestamp",
                lambda: self._reset_player_timestamp(item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.edit_pals.menu") if t else "Edit Pals",
                lambda: self._edit_player_pals(item.text(4), item.text(0)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.unlock_technologies.menu")
                if t
                else "Unlock All Technologies",
                lambda: self._unlock_all_technologies_for_player(item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.edit_tech_points") if t else "Edit Tech Points",
                lambda: self._edit_player_tech_points(),
            )
        )
        menu.add_action(
            self._create_action(
                t("player.edit_stats") if t else "Edit Player Stats",
                lambda: self._edit_player_stats(),
            )
        )
        menu.addSeparator()
        menu.add_action(
            self._create_action(
                "Set Player Level" if not t else t("player.set_level"),
                lambda: self._set_player_level(item.text(4)),
            )
        )
        menu.addSeparator()
        menu.add_action(
            self._create_action(
                t("guild.ctx.make_leader"),
                lambda: self._make_leader(item.text(6), item.text(4)),
            )
        )
        menu.add_action(
            self._create_action(
                t("deletion.ctx.delete_guild"), lambda: self._delete_guild(item.text(6))
            )
        )
        menu.add_action(
            self._create_action(
                t("guild.rename.menu"),
                lambda: self._rename_guild_action(item.text(6), item.text(5)),
            )
        )
        menu.add_action(
            self._create_action(
                t("guild.unlock_lab_research.menu") if t else "Unlock All Lab Research",
                lambda: self._unlock_all_lab_research_for_guild(item.text(6)),
            )
        )
        menu.add_action(
            self._create_action(
                t("guild.menu.max_level"), lambda: self._max_guild_level(item.text(6))
            )
        )
        menu.add_action(
            self._create_action(
                t("button.import"), lambda: self._import_base_to_guild(item.text(6))
            )
        )
    ...
    def _show_guild_context_menu(self, pos):
        ...
        menu.addAction(
            self._create_action(
                t("deletion.ctx.add_exclusion"),
                lambda: self._add_exclusion("guilds", item.text(1)),
            )
        )
        menu.addAction(
            self._create_action(
                t("deletion.ctx.remove_exclusion"),
                lambda: self._remove_exclusion("guilds", item.text(1)),
            )
        )
        menu.addAction(
            self._create_action(
                t("deletion.ctx.delete_guild"), lambda: self._delete_guild(item.text(1))
            )
        )
        menu.addAction(
            self._create_action(
                t("guild.rename.menu"),
                lambda: self._rename_guild_action(item.text(1), item.text(0)),
            )
        )
        menu.addAction(
            self._create_action(
                t("guild.menu.max_level"), lambda: self._max_guild_level(item.text(1))
            )
        )
        menu.addAction(
            self._create_action(
                t("guild.unlock_lab_research.menu") if t else "Unlock All Lab Research",
                lambda: self._unlock_all_lab_research_for_guild(item.text(1)),
            )
        )
        menu.addSeparator()
        menu.addAction(
            self._create_action(
                t("base.export_guild"),
                lambda: self._export_bases_for_guild(item.text(1)),
            )
        )
        menu.addAction(
            self._create_action(
                t("base.import_multi"), lambda: self._import_base_to_guild(item.text(1))
            )
        )
        menu.addAction(
            self._create_action(
    ...
    def _show_guild_member_context_menu(self, pos):
        ...
        menu.addAction(
            self._create_action(
                t("guild.ctx.make_leader"),
                lambda: self._make_leader(guild_data[1], item.text(4)),
            )
        )
        menu.addAction(
            self._create_action(
                t("guild.unlock_lab_research.menu") if t else "Unlock All Lab Research",
                lambda: self._unlock_all_lab_research_for_guild(guild_data[1]),
            )
        )
        menu.addSeparator()
        menu.addAction(
        ...
        menu.addAction(
            self._create_action(
                t("player.rename.menu"),
                lambda: self._rename_player(
                    item.text(4), item.text(0).replace("[L]", "")
                ),
            )
        )
        menu.addAction(
    ...
    def _show_base_context_menu(self, pos):
        ...
        menu.addAction(
            self._create_action(
                t("deletion.ctx.add_exclusion"),
                lambda: self._add_exclusion("bases", item.text(0)),
            )
        )
        menu.addAction(
            self._create_action(
                t("deletion.ctx.remove_exclusion"),
                lambda: self._remove_exclusion("bases", item.text(0)),
            )
        )
        menu.addAction(
            self._create_action(
                t("deletion.ctx.delete_base"),
                lambda: self._delete_base(item.text(0), item.text(1)),
            )
        )
        menu.addAction(
            self._create_action(
                t("guild.rename.menu"),
                lambda: self._rename_guild_action(item.text(1), item.text(2)),
            )
        )
        menu.addAction(
            self._create_action(
        ...
        menu.addAction(
            self._create_action(
                t("export.base"), lambda: self._export_base(item.text(0))
            )
        )
        menu.addAction(
            self._create_action(
                t("base.radius.menu") if t else "Adjust Radius",
                lambda: self._adjust_base_radius(item.text(0)),
            )
        )
        menu.addAction(
            self._create_action(
                t("import.base"), lambda: self._import_base(item.text(1))
            )
        )
        menu.addAction(
            self._create_action(
                t("clone.base"), lambda: self._clone_base(item.text(0), item.text(1))
            )
        )
    ...
    def _show_exclusion_context_menu(self, pos, excl_type):
        ...
        menu.addAction(
            self._create_action(
                t("deletion.ctx.remove_exclusion"),
                lambda: self._remove_exclusion(excl_type, item.text(0)),
            )
        )
    ...
    def _rename_world(self):
        ...
        old = (
            meta_gvas.properties.get("SaveData", {})
            .get("value", {})
            .get("WorldName", {})
            .get("value", "Unknown World")
        )
        new_name = InputDialog.get_text(
            t("world.rename.title"), t("world.rename.prompt", old=old), self
        )
        if new_name:
...
>>>>>>> Stashed changes
    def _load_worldoption(self):
        ...
        sav_path, _ = QFileDialog.getOpenFileName(
            ...
            t("menu.file.load_worldoption") if t else "Load WorldOption",
            "",
            "WorldOption.sav (WorldOption.sav)",
        )
        if not sav_path:
            return
        if not os.path.basename(sav_path).startswith("WorldOption"):
            self._show_warning(
                ...
                "Please select a WorldOption.sav file",
            )
            return
        try:
            json_data = sav_to_json(sav_path)
            if "properties" not in json_data or "OptionWorldData" not in json_data.get(
                "properties", {}
            ):
                self._show_warning(
                    t("error.title") if t else "Error",
                    "Invalid WorldOption.sav structure",
                )
                return
            from palworld_aio.editors.worldoption_editor import (
                edit_worldoption_settings,
            )
...
<<<<<<< Updated upstream
    ...
    def _load_levelmeta(self):
        ...
        sav_path, _ = QFileDialog.getOpenFileName(
            ...
            t("menu.file.load_levelmeta") if t else "Load LevelMeta",
            "",
            "LevelMeta.sav (LevelMeta.sav)",
        )
        if not sav_path:
            return
        if not os.path.basename(sav_path).startswith("LevelMeta"):
            self._show_warning(
                t("error.title") if t else "Error", "Please select a LevelMeta.sav file"
            )
            return
        try:
            json_data = sav_to_json(sav_path)
            if "properties" not in json_data or "SaveData" not in json_data.get(
                "properties", {}
            ):
                self._show_warning(
                    t("error.title") if t else "Error",
                    "Invalid LevelMeta.sav structure",
                )
                return
            from palworld_aio.editors.levelmeta_editor import edit_levelmeta_settings

            result = edit_levelmeta_settings(json_data, sav_path, self)
            if result:
                self._show_info(
                    t("success.title") if t else "Success",
                    f"LevelMeta settings saved successfully!\n\nLocation: {sav_path}",
                )
        except Exception as e:
            self._show_error(
                t("error.title") if t else "Error",
...
>>>>>>> Stashed changes
    ...
    def _delete_inactive_bases(self):
        ...
        days = DaysInputDialog.get_days(
            ...
            t("deletion.inactive_bases.prompt")
            if t
            else "Delete bases where ALL players inactive for how many days?",
            self,
        )
        if not days:
            return
        ...
        def on_finished(cnt):
            ...
            msg_box.setText(
                t("bases.deleted_count", count=cnt) if t else f"Deleted {cnt} bases"
            )
            msg_box.addButton(t("button.ok") if t else "OK", QMessageBox.AcceptRole)
            msg_box.exec()
    ...
    def _delete_inactive_players(self):
        if not constants.loaded_level_json:
            ...
            return
        days = DaysInputDialog.get_days(
            t("deletion.inactive_days_title"), t("deletion.inactive_days_prompt"), self
        )
        if days:
            ...
            def on_finished(removed):
                self.refresh_all()
                self._show_info(
                    t("Done"), t("deletion.inactive_players_removed", count=removed)
                )
    ...
    def _delete_non_base_map_objs(self):
        ...
        def on_finished(removed):
            self.refresh_all()
            self._show_info(
                t("Done"), t("deletion.non_base_objs_removed", count=removed)
            )
    ...
    def _remove_invalid_passives(self):
        ...
        def on_finished(removed):
            self.refresh_all()
            self._show_info(
                t("Done"), t("deletion.invalid_passives_removed", count=removed)
            )
    ...
    def _fix_illegal_pals(self):
        ...
        reply = show_question(
            self,
            t("Confirm") if t else "Confirm",
            t("deletion.fix_illegal_pals_confirm")
            if t
            else "This will fix all illegal pals by setting their stats to legal maximums (level 65, IVs 100, souls 20). Continue?",
        )
        if not reply:
            return
        ...
        def on_finished(fixed):
            self.refresh_all()
            self._show_info(
                t("Done") if t else "Done",
                t("deletion.illegal_pals_fixed", count=fixed)
                if t
                else f"Fixed {fixed} illegal pals to legal maximums.",
            )
    ...
    def _fix_all_timestamps(self):
        ...
        self._show_info(
            t("Done") if t else "Done",
            t("timestamps.fixed_count", count=fixed)
            if t
            else f"Fixed {fixed} player timestamps",
        )
    ...
    def _reset_player_timestamp(self, uid):
        if reset_selected_player_timestamp(uid, self):
            ...
            self._show_info(
                t("Done") if t else "Done",
                t("timestamps.player_reset")
                if t
                else "Player timestamp reset to current time",
            )
        else:
            self._show_warning(
                t("Error") if t else "Error",
                ...
                if t
                else "Failed to reset player timestamp",
            )
    ...
    def _move_player_to_guild(self):
        ...
        if move_player_to_guild(player_data[4], guild_data[1]):
            ...
            self._show_info(
                t("Done"),
                t("guild.move.moved", player=player_data[0], guild=guild_data[0]),
            )
        else:
    ...
    def _generate_map(self):
        ...
        def on_finished(path):
            if path:
                ...
                msg_box.setWindowTitle(t("Done") if t else "Done")
                msg_box.setText(
                    t("map_saved", path=path) if t else f"Map saved to {path}"
                )
...
>>>>>>> Stashed changes
    ...
    def _refresh_texts(self):
        ...
        self.setWindowTitle(
            t("app.title", version=tools_version) + " - " + t("tool.deletion")
        )
        if hasattr(self, "results_widget") and self.results_widget:
        ...
        if detach_btn:
            detach_btn.setText(
                t("console.reattach")
                if self.status_stream and self.status_stream.detached
                else t("console.detach")
            )
    ...
    def _add_exclusion(self, excl_type, value):
        if value not in constants.exclusions[excl_type]:
            ...
            self._show_info(
                t("Info"),
                t(
                    "deletion.info.already_in_exclusions",
                    kind=excl_type[:-1].capitalize(),
                ),
            )
    ...
    def _delete_player(self, uid):
        if uid in constants.exclusions.get("players", []):
            self._show_warning(
                ...
                t("deletion.warning.protected_player")
                if t
                else f"Player {uid} is in exclusion list and cannot be deleted.",
            )
            return
        delete_player(uid)
        self.refresh_all()
    ...
    def _delete_guild(self, gid):
        if gid in constants.exclusions.get("guilds", []):
            self._show_warning(
                t("warning.title") if t else "Warning",
                t("deletion.warning.protected_guild")
                if t
                else f"Guild {gid} is in exclusion list and cannot be deleted.",
            )
            return
        delete_guild(gid)
        self.refresh_all()
    ...
    def _delete_base(self, bid, gid):
        if bid in constants.exclusions.get("bases", []):
            self._show_warning(
                t("warning.title") if t else "Warning",
                t("deletion.warning.protected_base")
                if t
                else f"Base {bid} is in exclusion list and cannot be deleted.",
            )
            return
    ...
    def _rename_player(self, uid, old_name):
        new_name = InputDialog.get_text(
            t("player.rename.title"), t("player.rename.prompt"), self
        )
        if new_name:
            rename_player(uid, new_name)
            self.refresh_all()
            self._show_info(
                t("player.rename.done_title"),
                t("player.rename.done_msg", old=old_name, new=new_name),
            )
    ...
    def _rename_guild_action(self, gid, old_name):
        new_name = InputDialog.get_text(
            t("guild.rename.title"), t("guild.rename.prompt"), self
        )
        if new_name:
            rename_guild(gid, new_name)
            self.refresh_all()
            self._show_info(
                t("guild.rename.done_title"),
                t("guild.rename.done_msg", old=old_name, new=new_name),
            )
    ...
    def _import_base_to_guild(self, gid):
        file_paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Base JSON Files", "", "JSON Files(*.json)"
        )
        if not file_paths:
            return
            ...
            for base in bases:
                ...
                try:
                    ...
                    if not data:
                        ...
                        continue
                    safe_gname = "".join(
                        (c for c in gname if c.isalnum() or c in (" ", "-", "_"))
                    ).rstrip()
    ...
    def _export_bases_for_guild(self, gid):
        ...
        if not guild_bases:
            self._show_info(
                t("Info") if t else "Info", f'No bases found for guild "{guild_name}".'
            )
            return
        export_dir = QFileDialog.getExistingDirectory(
            self, f'Select Export Directory for "{guild_name}"'
        )
        if not export_dir:
            return
        successful_exports = 0
        ...
        if successful_exports > 0:
            ...
            self._show_warning(
                t("error.title"),
                f'Failed to export any bases for guild "{guild_name}".\n'
                + "\n".join(failed_bases),
            )
    ...
    def _export_base(self, bid):
        ...
        if not data:
            self._show_warning(
                t("error.title") if t else "Error",
                t("base.export.not_found")
                if t
                else f"Could not find base data for ID: {bid}",
            )
            return
    ...
    def _adjust_base_radius(self, bid):
        ...
        src_base_entry = next(
            (
                b
                for b in base_camp_data
                if str(b["key"]).replace("-", "").lower()
                == bid.replace("-", "").lower()
            ),
            None,
        )
        if not src_base_entry:
            ...
            return
        current_radius = src_base_entry["value"]["RawData"]["value"].get(
            "area_range", 3500.0
        )
        new_radius = RadiusInputDialog.get_radius(
            t("base.radius.title") if t else "Adjust Base Radius",
            t("base.radius.prompt")
            if t
            else f"Current radius: {int(current_radius)}\nEnter new radius (50% -1000%):",
            current_radius,
            self,
        )
        if new_radius is not None and new_radius != current_radius:
            if update_base_area_range(constants.loaded_level_json, bid, new_radius):
                ...
                self._show_info(
                    t("success.title") if t else "Success",
                    t("base.radius.updated", radius=int(new_radius))
                    if t
                    else f"Base radius updated to {new_radius}\n\n⚠ Load this save in-game for structures to be reassigned.",
                )
            else:
                self._show_error(
                    t("error.title") if t else "Error",
                    t("base.radius.failed") if t else "Failed to update base radius",
                )
    ...
    def _trim_overfilled_inventories(self):
        ...
        def on_finished(fixed):
            ...
            self._show_info(
                t("done") if t else "Done",
                t("deletion.trimmed_inventories", fixed=fixed)
                if t
                else f"Trimmed {fixed} overfilled inventories",
            )
    ...
    def _unlock_all_technologies_for_player(self, uid):
        if unlock_all_technologies_for_player(uid, self):
            self._show_info(
                t("Done") if t else "Done",
                t("player.unlock_technologies.success")
                if t
                else "Unlock All Technologies completed",
            )
        else:
            self._show_warning(
                t("Error") if t else "Error",
                t("player.unlock_technologies.failed")
                if t
                else "Unlock All Technologies failed",
            )
    ...
    def _unlock_all_lab_research_for_guild(self, gid):
        if unlock_all_lab_research_for_guild(gid, self):
            self._show_info(
                t("Done") if t else "Done",
                t("guild.unlock_lab_research.success")
                if t
                else "Unlock All Lab Research completed",
            )
        else:
            self._show_warning(
                t("Error") if t else "Error",
                t("guild.unlock_lab_research.failed")
                if t
                else "Unlock All Lab Research failed",
            )
    ...
    def _level_up_player(self, uid):
        ...
        if current_level == 1 or current_level == "?":
            self._show_warning(
                t("Error") if t else "Error",
                t("player.level.set_no_level_data")
                if t
                else "Cannot level up player - player is at level 1 or unknown",
            )
            return
        if adjust_player_level(uid, current_level + 1):
            self.refresh_all()
            self._show_info(
                t("Done") if t else "Done", "Player leveled up successfully"
            )
        else:
            self._show_warning(
                t("Error") if t else "Error",
                "Failed to level up player (already max level?)",
            )
    ...
    def _level_down_player(self, uid):
        ...
        if current_level == 1 or current_level == "?":
            self._show_warning(
                ...
                t("player.level.set_no_level_data")
                if t
                else "Cannot level down player - player is at level 1 or unknown",
            )
            return
        if current_level - 1 < 2:
            self._show_warning(
                t("Error") if t else "Error",
                t("player.level.minimum_level")
                if t
                else "Cannot level down player - minimum level is 2",
            )
            return
        if adjust_player_level(uid, current_level - 1):
            self.refresh_all()
            self._show_info(
                t("Done") if t else "Done", "Player leveled down successfully"
            )
        else:
            self._show_warning(
                t("Error") if t else "Error",
                "Failed to level down player (already min level?)",
            )
    ...
    def _set_player_level(self, uid):
        ...
        if current_level == 1 or current_level_raw == "?":
            self._show_warning(
                ...
                t("player.level.set_no_level_data")
                if t
                else "Cannot set player level - player is at level 1 or unknown",
            )
            return
        new_level = LevelInputDialog.get_level(
            t("player.set_level.title") if t else "Set Player Level",
            t("player.set_level.prompt", current_level=current_level)
            if t
            else f"Current level: {current_level}\nEnter new level (2-65):",
            current_level,
            self,
        )
        if new_level is not None and new_level != current_level:
            if new_level < 2:
                self._show_warning(
                    ...
                    t("player.level.minimum_level")
                    if t
                    else "Cannot set player level - minimum level is 2",
                )
                return
            if adjust_player_level(uid, new_level):
                ...
                self._show_info(
                    t("Done") if t else "Done",
                    t("player.level.set_success", level=new_level)
                    if t
                    else f"Player level set to {new_level}",
                )
            else:
                self._show_warning(
                    t("Error") if t else "Error",
                    t("player.level.set_failed") if t else "Failed to set player level",
                )
    ...
    def _modify_container_slots(self):
        if not constants.loaded_level_json:
            self._show_warning(t("Error"), t("error.no_save_loaded"))
            return
        new_slot_num, ok = QInputDialog.getInt(
            self,
            t("modify_container_slots_title") if t else "Modify Container Slots",
            t("modify_container_slots_prompt")
            if t
            else "Enter new slot number for all containers:",
            50,
            1,
            1000,
            1,
        ...
        if ok:
            ...
            def on_finished(modified):
                ...
                self._show_info(
                    t("Done"),
                    t("modify_container_slots_result", modified=modified)
                    if t
                    else f"Modified {modified} containers",
                )
...
>>>>>>> Stashed changes
    def _edit_player_tech_points(self):
        ...
        sav_file = os.path.join(
            constants.current_save_path, "Players", f"{uid_clean}.sav"
        )
        try:
            gvas = sav_to_gvasfile(sav_file)
            current_tech = (
                gvas.properties.get("SaveData", {})
                .get("value", {})
                .get("TechnologyPoint", {})
                .get("value", 0)
            )
        ...
        dialog.setWindowTitle(
            t("player.edit_tech_points.title") if t else "Edit Technology Points"
        )
        ...
        boss_tech_label = QLabel(
            t("player.ancient_tech_points") if t else "Ancient Technology Points:"
        )
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            from ..player_manager import (
                set_player_tech_points,
                set_player_boss_tech_points,
            )
            ...
            if success:
                self._show_info(
                    t("Done"),
                    t(
                        "player.tech_points_both_updated",
                        new_tech=new_tech,
                        new_boss_tech=new_boss_tech,
                    ),
                )
            else:
    ...
    def _edit_player_stats(self):
                ...
                if "GotStatusPointList" in sp_val:
                    ...
                    for status_item in got_status_list:
                        if "StatusName" in status_item and "StatusPoint" in status_item:
                            stat_name_jp = (
                                status_item["StatusName"].get("value", "")
                                if isinstance(status_item.get("StatusName"), dict)
                                else ""
                            )
                            stat_point = (
                                status_item["StatusPoint"].get("value", 0)
                                if isinstance(status_item.get("StatusPoint"), dict)
                                else 0
                            )
                            current_stats[stat_name_jp] = stat_point
                if "UnusedStatusPoint" in sp_val:
                    unused_stat_points = (
                        sp_val["UnusedStatusPoint"].get("value", 0)
                        if isinstance(sp_val.get("UnusedStatusPoint"), dict)
                        else 0
                    )
                break
        stat_names = {}
        stat_name_map = {
            "最大HP": "player.stats.max_hp",
            "最大SP": "player.stats.max_sp",
            "攻撃力": "player.stats.attack_power",
            "所持重量": "player.stats.carry_weight",
            "捕獲率": "player.stats.capture_rate",
            "作業速度": "player.stats.work_speed",
        }
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle(
            t("player.edit_stats.title") if t else "Edit Player Stats"
        )
        dialog.setModal(True)
        layout = QVBoxLayout(dialog)
        stat_widgets = {}
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if set_player_stats(uid, new_stats, new_unused_stat_points):
                self._show_info(
                    t("Done"),
                    t("player.stats_updated")
                    if t
                    else "Player stats updated successfully",
                )
            else:

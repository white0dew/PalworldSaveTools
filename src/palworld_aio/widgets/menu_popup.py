...
class ScrollableMenu(QWidget):
    ...
    def _update_theme(self):
        if self.is_dark:
            ...
            hover_color = "#1e3a8a"
        self.setStyleSheet(
            f"QWidget {{ background: {bg}; border: 1px solid {border}; border-radius: 10px; }} QPushButton {{ background: transparent; border: none; padding: 8px 12px; text-align: left; color: {text_color}; }} QPushButton:hover {{ background: {hover_bg}; color: {hover_color}; }}"
        )
...
class MenuPopup(QWidget):
    ...
    def _check_cursor_position(self):
        cursor_pos = (
            QGuiApplication.primaryScreen().availableGeometry().topLeft()
            + QCursor.pos()
        )
        ...
        over_submenu = self._current_menu and self._is_point_in_widget(
            cursor_pos, self._current_menu
        )
    ...
    def _setup_ui(self):
        ...
        categories = [
            ...
            (
                "functions",
                "nf-md-function",
                t("deletion.menu.delete") if t else "Functions",
            ),
            ("maps", "nf-md-map", t("deletion.menu.view") if t else "Maps"),
            (
                "exclusions",
                "nf-md-playlist_remove",
                t("deletion.menu.exclusions") if t else "Exclusions",
            ),
            ("aio", "nf-md-update", t("aio.menu.updates") if t else "Updates"),
        ...
        if category == "functions":
            ...
            for item in actions:
                ...
                action.triggered.connect(
                    lambda checked, cb=callback: self._on_menu_action(cb)
                )
            btn_pos = button.mapToGlobal(QPoint(button.width(), 0))
            self._current_menu = menu
            self._current_category = category
    ...
    def refresh_labels(self):
        labels = {
            ...
            "aio": t("aio.menu.updates") if t else "Updates",
